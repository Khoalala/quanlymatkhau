...
public class InscriptionUI extends JFrame implements ActionListener{
	...
	private void initialize() {
		...
		lblSoumgraphic.setForeground(new Color(125, 128, 144));
		lblSoumgraphic.setFont(new Font("Tahoma", Font.BOLD, 16));
		lblSoumgraphic.setBounds(125, 282, 272, 27);
		...
		JLabel lblNewLabel = new JLabel("Lưu trữ và giữ mật khẩu an toàn");
		lblNewLabel.setFont(new Font("Tahoma", Font.BOLD, 13));
		lblNewLabel.setForeground(new Color(255, 255, 255));
		lblNewLabel.setBounds(51, 36, 320, 16);
		panel_1.add(lblNewLabel);
		
		JLabel lblEnterYourDetails = new JLabel("Nhập thông tin liên hệ của bạn");
		...
		notRobotRdBtn = new JRadioButton("Tôi không phải là robot *");
		...
		passwordTxtfield.setBounds(51, 284, 282, 16);
		passwordTxtfield.setEchoChar('*');
		panel_1.add(passwordTxtfield);
		
		JButton inscriptionBtn = new JButton("Đi nào !");
		...
		JLabel iconViewOrHidePasswordEditLbl = new JLabel("");
		iconViewOrHidePasswordEditLbl.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		iconViewOrHidePasswordEditLbl.setIcon(new ImageIcon(InscriptionUI.class.getResource("/ui/images/eye_white.png")));
		...
		iconViewOrHidePasswordEditLbl.setBounds(345, 281, 23, 23);
				...
				if (Utils.validateEmail(emailUser)) {
					if (Utils.checkStringMinLength(pwdUser, 6)) {
						if (notRobotRdBtn.isSelected()) {
							...
							Utils.showErrorMessage(frame, "Bạn có phải là robot không ? chọn bên dưới nếu bạn không phải!");
							return false;
						}
					}else {
						Utils.showErrorMessage(frame, "Mật khẩu phải dài ít nhất 6 ký tự");
						return false;
					}
				}else {
					Utils.showErrorMessage(frame, "Vui lòng nhập địa chỉ email hợp lệ!");
					return false;
				}
				
			}else {
				Utils.showErrorMessage(frame, "Tên phải có ít nhất 2 ký tự");
				return false;
			}
			
		}else {
			Utils.showErrorMessage(frame, "Vui lòng điền vào tất cả các ô!");
			return false;
		}
	}
