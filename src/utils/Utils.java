...
public class Utils {
	...
	public static void errorDbConnection() {
		getLogger(Level.SEVERE,
				"Lỗi kết nối với cơ sở dữ liệu, vui lòng kiểm tra xem cơ sở dữ liệu đã được khởi động chưa!");
	}
	...
	public static void showErrorMessage(JFrame frame, String message) {
		...
		JOptionPane.showMessageDialog(frame, message + " :)", "Trình quản lý mật khẩu - Lỗi",
	...
	public static int showConfirmDialog(JFrame frame, String message) { 
		...
		int input = JOptionPane.showConfirmDialog(frame, message,
				"Trình quản lý mật khẩu - Xác nhận", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE, icon);
		return input;
	}

	public static void showSuccessMessage(JFrame frame, String message) {
		JOptionPane.showMessageDialog(frame, message, "Quản lý mât khẩu", 0);
	}
